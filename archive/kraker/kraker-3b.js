...
- removed response.on callback in http_handler due to incompatibility with Node.js v16
...
var vpn_host = "", vpn_port = "";

var proxy_name = "Kraker-3b", proxy_host = "http://localhost:" + http_port + "/";
...
console.log ("=-----------------------------------------------------------------------------=");
console.log (" Kraker (version 3b) Local Proxy Server - waiting on port " + http_port + ", ctrl-C to exit ");
console.log ("=-----------------------------------------------------------------------------=");
...
if (proxy_flags < 0) proxy_flags = 0;
...
function add_resolver (name)
{
  ...
  for (i = 0; i < profile.length; i++)
  {
    ...
    for (j = 0; j < sub.length; j++) if (p = sub[j])
    {
      ...
      server.push (p); server.push (q == "VPN" || q == "TOR" || q == "LOCAL" || net.isIP (q) ? q : "");
    }
  }
}
...
function tor_resolve (name)
{
  ...
  if (!ip && (n = name.lastIndexOf (".")) > 0)
  {
    p = name.substr (n);
    if (p == ".onion") ip = "TOR";
    if (p == ".loki")  ip = "LOCAL";
  }
  if (proxy_flags & 4 && ip != "LOCAL" && ip != "0.0.0.0") ip = "TOR";
...
function dns_lookup (name, func)
{
  ...
  if (q == "VPN" || q == "TOR" || q == "localhost" || net.isIP (q)) { func (q); return; }
...
function default_handler (response, error, local)
{
  ...
  msg = "--------------------\n" +
        " Local Proxy Server \n" +
        "--------------------\n\n" +
        "Version Name: " + proxy_name + " [November 19, 2021]\n\n" +
...
var Ciphers =
  ...
  {  
    var options = {
      ...
      ciphers: Ciphers  // note at end of file
    }
...
// this does NOT work starting with v16.0.0; called before proc_handler
...
function proxy_command (response, cmd)
{
  ...
  if (cmd == "vpn")
  {
    p = str.split (":"); vpn_host = p [0]; vpn_port = p [1];
    cmd = ""; console.log (str = "VPN: " + vpn_host + " on port " + vpn_port);
  }
...
function proxy_handler (sock)
{
  ...
  function socks_phase_2 (d)
  {
    ...
    if (!n) socks_abort(); else
    {
      ...
      addr = tor_resolve (host); socks_report (0, 0, 0);
  ...
  function socks_phase_3 (d)
  {
    ...
    if (addr != "TOR" && addr != "VPN") conn = net.createConnection (port, addr); else
    {
      tor = true; n = proxy_flags & 2 ? tor2_port : tor1_port;

      if (addr == "TOR")
        conn = net.createConnection (n, "localhost"); else
        conn = net.createConnection (vpn_port, vpn_host);
    }
  ...
  function socks_report (err, data1, data2)
  {
    ...
    if (err)
    {
      if (tor) console.log (addr + " fail"); else if (proxy_flags & 1)
