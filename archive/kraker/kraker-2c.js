...
- added dns_lookup for 8080 and 8088
...
var proxy_name = "Kraker-2c", proxy_host = "http://localhost:" + http_port + "/";
...
var http_server = http.createServer (http_handler).listen (http_port);
https.createServer (ssl, https_handler).listen (https_port);

console.log ("=-----------------------------------------------------------------------------=");
console.log (" Kraker (version 2c) Local Proxy Server - waiting on port " + http_port + ", ctrl-C to exit ");
console.log ("=-----------------------------------------------------------------------------=");

var proxy_tunnel = null, proxy_flags = profile_count = -1, dns_count = 0;
var sockets_open = 0, sockets_count = 9, server = [], profile = [], dnslist = [];
...
function dns_servers (name)
{
  ...
  if (!this.original) this.original = dns.getServers();
...
{
  var n, p, q = name; if (name == "TOR" || net.isIP (name)) { func (name); return; }
  ...
  for (n = dnslist.length - 2; n >= 0; n -= 2) if (name == dnslist [n])
  {
    p = dnslist [n + 1].split (" ");
    q = p [0]; n = time_now - (dns_count ? 300 : 0);
    if (p [1] * 1 < n) break; else { func (q); return; }
  }

  if (dns_count) setTimeout (function () { dns_lookup (name, func); }, 250); else
  {
    dns_count++; dnslist.push (name); dnslist.push (" ");

    dns.resolve4 (name, function (err, list)
    {
      if (err) console.log (">> DNS:", err.code, name); else q = list [0];
      n = dnslist.length - 1; time_now = Math.trunc (Date.now() / 1000);
      dnslist [n] = q + " " + (time_now + (err ? 60 : 300));
      dns_count--; func (q);
    });
  }
}
...
function default_handler (response, error, local)
{
  ...
  msg = "--------------------\n" +
        " Local Proxy Server \n" +
        "--------------------\n\n" +
        "Version Name: " + proxy_name + " [April 8, 2021]\n\n" +
...
//  if (local < -1 || local > 1) myheader ["accept-encoding"] = "identity";   // no compression
  ...
  dns_lookup (dns_resolve (host), function (addr) { http_process (addr); });
...
function proxy_handler (sock)
{
  ...
  sock.on ("close", function () { --sockets_open; socks_report (0,1); socks_abort(); });
  ...
  function socks_phase_2 (d)
  {
    ...
    if (!n) socks_abort(); else
    {
      ...
      if (proxy_flags & 4) addr = "TOR"; else addr = tor_resolve (host);
      if (proxy_flags & 1) socks_report (0,0);

      dns_lookup (addr, function (ip) { addr = ip; socks_phase_3 (d); });
    }
  }
  ...
  function socks_phase_3 (d)
  {
    ...
    conn.on ("error",   function(e) { socks_report (e.code, e.address); });
    conn.on ("close",   function () { socks_report (0,2); socks_abort(); });
    conn.on ("end",     function () { });  // this callback is needed for the "close" event
  }

  function socks_phase_4 (d)
  {
    if (!tor) { socks_phase_5 (null); return; }
  ...
  function socks_report (err, data)
  {
    if (err)
    {
      if (tor) console.log ("TOR fail"); else
      {
        var s = ">>" + (proxy_flags & 16 ? " " + socket + " -" : "");
        if (proxy_flags & 1) console.log (s, "ERR:", err, data ? data : "");
      }
      return;
    }
    ...
    if (!(proxy_flags & 16))
    {
      if (data == 0) console.log (">> " + host + " <" + port + "> " + (addr != host ? addr : ""));
      return;
    }

    if (data == 0) console.log (">> " + socket + " - " + host + " <" + port + "> " + (addr != host ? addr : ""));
    if (data == 1) console.log (" @", socket, "-", sockets_open, conn ? "" : "(cancelled)");
    if (data == 2) console.log (" :", socket, "-", sock.readyState);
  }
}
