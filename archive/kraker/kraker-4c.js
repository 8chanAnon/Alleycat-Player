...
var proxy_name = "Kraker-4c", proxy_host = "http://localhost:" + http_port + "/";
...
console.log ("=-----------------------------------------------------------------------------=");
console.log (" Kraker (version 4c) Local Proxy Server - waiting on port " + http_port + ", ctrl-C to exit ");
console.log ("=-----------------------------------------------------------------------------=");
...
function http_handler (request, response)
{
  ...
  {
    shadow_on = n; if (m [0] == "$") m = m.substr (1);
...
function proc_handler (response, res, config, local)
{
  ...
  if (config.shadow != "@" && (v = res.headers [s = "location"]))
  {
    ...
    if (z.includes ("/localhost:")) z += config.referral + config.headers; else
    {
