...
var proxy_name = "Kraker-4e", proxy_addr = "127.0.0.1";
...
var https_server = start_https_server ("", "");
...
console.log ("=-----------------------------------------------------------------------------=");
console.log (" Kraker (version 4e) Local Proxy Server - waiting on port " + http_port + ", ctrl-C to exit ");
console.log ("=-----------------------------------------------------------------------------=");
  ...
  for (i = 0; i < profile.length; i++)
  {
    ...
    for (j = 0; j < sub.length; j++) if (p = sub[j])
    {
      ...
      if (r == "SHD")
      {
        ...
        if (p [0] == "+") { s += "+"; p = p.substr (1); }
        if (p [0] == "~") { s += "~"; p = p.substr (1); }
        s += q.substr (4); r = s [0] == "$" ? ":443" : ":80";
        if (s.substr (-1) == "/") s = s.substr (0, s.length - 1);
        if (!p.includes (":")) p += r; shadow_host [p] = s; continue;
      }
...
function dns_resolve (name, vpx)
{
  ...
  if (!ip)
  {
    ...
    if (p == "localhost") return (proxy_addr); if (n < 0) return ("LOCAL");
...
function default_handler (response, error, local)
{
  ...
  msg = "--------------------\n" +
        " Local Proxy Server \n" +
        "--------------------\n\n" +
        "Version Name: " + proxy_name + " [November 9, 2022]\n\n" +
...
{
  var header = {}; header ["zz-proxy-server"] = proxy_name;
  ...
  {
    if (local > 0) console.log (" Local Request: " + msg);
...
function local_data (name, data)
{
  if (name) if (data)
  {
    ...
    if (local_files.length > 150) local_files.splice (0,10);
...
{
  var m, n, p = "", q = u [0] == "?"; if (q) u = u.substr (1);

  n = u.indexOf ("/+"); if (n >= 0) u = u.substr (n + 1);
  ...
  if (u [0] == "+")
  {
    ...
    if (local > 0) console.log (" FILE: " + (u ? u : "none"));
    if (u && q == 3) u = "?" + u;
  }
  else
  {
    m = u.toLowerCase();
    if (m.substr (-1) == "." || m.includes ("./")) u = "";
    if (m [0] == "/" || m == aliases || m == settings) u = "";
  }
  return (u);
}
...
function http_handler (request, response)
{
  ...
  url = url.replace (/%7C/g, "|").replace (/\|\|/g, "%7C");
  ...
  if (m && (url == "@" || m [2] == ""))
  {
    if (m [1] != shadow_secret)
    {
      console.log (shadow_host); m = " See the console for your info.";
      proc_done (response, m, "text/plain", 0); return;
    }
    var i, j, ii = 0, jj = 0, p, q; m = Object.entries (shadow_host);

    for (n = 0; n < m.length; n++)
    {
      p = m[n][0]; i = p.length; j = p.indexOf (":");
      if (i > ii) ii = i; if (j > jj) jj = j;
    }
    for (n = 0; n < m.length; n++)
    {
      p = m[n][0].replace (":", " ");
      j = jj - p.indexOf (" ") + 1; i = ii - p.length - j + 3;
      refer += " ".repeat (j) + p + " ".repeat (i) + "(" + m[n][1] + ")\n";
    }
    proc_done (response, refer, "text/plain", 0); return;
  }
  ...
  if ((m = url.indexOf ("$") + 1) > 0 && (n = url.indexOf ("$", m) + 1) > 1)
  {
    ...
    if (m && m == shadow_secret) url = "\\" + n; else if (url [0] == "$")
    {
      ...
      if (p == "" && m.indexOf (".") <= 0) { shadow_on = m; url = n; }
    }
  }

  n = shadow_on || shadow; shadow_on = request.headers ["origin"] || "*";
  m = n.indexOf (".shadow.localhost:"); if (m > 0) n = n.substr (0, m);
  ...
  if (url [0] == "\\")
  {
    ...
    if (url = url.substr (m)) shadow_on = "."; else
    {
  ...
  {
    if (m [0] == "$") m = m.substr (1); if (m) referral = "@";
    if (m [0] == "+") { shadow_on = "."; m = m.substr (1); referral = ""; }
    m = m.replace (/\$\$\$/g, n.substr (0, n.indexOf (":")));
    if (m) url = m + (url ? "/" + url : "");
  }

  if (shadow [0] == "@") referral = "@"; else
  {
    local = 1; if (url [0] == "~") { local = -1; referral += "~"; url = url.substr (1); }
  ...
  if (method == "OPTIONS" && referral [0] != "@") { options_proc (request, response); return; }
  ...
  n = url.indexOf ("?"); m = safe_decode (n < 0 ? url : url.substr (0, n));

  if (!m || shadow_on == "." || (referral [0] != "@" && !m.includes (":")))
  {
    if (!(url = m)) default_handler (response, 888, local);
    else if (method == "GET") get_file (request, response, url, local);
    else if (method == "PUT") put_file (request, response, url, local);
    else if (method == "POST") handle_special (request, response, url, local, shadow);
  ...
  {
    // access key for cookies
    n = url.length; if (m = param ["key"]) url = url.replace ("$" + m + "$", "");
    if (shadow_on != "*" && n == url.length && m != "") shadow_on = "$";

    // for redirection and tweak_m3u8
    if ((head1 = referral + head1)[0] == "@") { head1 = ""; shadow_on = "@" + shadow_on; }

    // don't pass localhost through the socks5 port and don't redirect back to here
    if (host.substr (host.lastIndexOf (".") + 1) == "localhost") host = proxy_addr;
    if (host == proxy_addr) { shadow = "@"; port = portnum; }
  }

  n = local > 0 ? ++reqcount : 0; m = param ["vpx"];
  m = (port ? ":" + host : "") + (m ? ":" + m : "");
...
function proc_handler (response, res, config, local)
{
  ...
  if (local <= 0 || config.method == "OPTIONS") header = Object.assign (res.headers); else
  {
    var header_name = [
  ...
  var origin = config.origin, is_localhost = origin [0] != "@";
  origin = is_localhost ? "*" : origin.substr (1);

  v = header ["access-control-allow-credentials"] == "true" ? origin : "*";
  s = "access-control-allow-origin"; if (v == "$") v = header [s] || "*";
  header [s] = v; header ["zz-proxy-server"] = proxy_name;
  ...
  if (config.shadow != "@" && (v = res.headers [s = "location"]))
  {
    ...
    if (is_localhost) z += config.headers; else
    {
  ...
  s = "set-cookie"; v = res.headers [s]; if (is_localhost) delete header [s];
  ...
  if (config.shadow == "@" || (local > -2 && local < 2))
  {
...
function proxy_command (response, cmd, ssl)
{
  ...
  if (cmd == "restart") if (ssl) { cmd = ""; str = "Bad idea"; } else
  {
    ...
    {
      str = (str + "+").split ("+");
      https_server = start_https_server (str[1].trim(), str[0].trim());
...
function proxy_handler (sock)
{
  ...
  function socks_phase_4 (d)
  {
    ...
    // v4e: issue with IPv6 in Node.js 18
    if (vhost == "localhost") vhost = proxy_addr;
    if (vhost == proxy_addr && vport == socks_port) { socks_abort(); return; }
...
function tweak_m3u8 (data, config)
{
  ...
  // version 4e: browser tends to strip out double dots so use ! instead
  fix1 = fix1.replace (/!/g, "."); fix2 = fix2.replace (/!/g, ".");
  ...
  var myfix = config.shadow + config.headers;
  ...
  if (fix2.substr (0,1) == "/") myfix = myfix + config.host;
